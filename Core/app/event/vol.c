...
uint8_t Vol_Event_Handle(Vol_t *vol){
	...
	if(vol->vol_src[VOL_A] == VOL_FROM_MEM){
		if (_abs( vol->vol_raw[VOL_A] - vol->vol_pre[VOL_A] ) >= VOL_TOLERANCE){
	...
	if(vol->vol_src[VOL_B] == VOL_FROM_MEM){
		if (_abs( vol->vol_raw[VOL_B] - vol->vol_pre[VOL_B] ) >= VOL_TOLERANCE){
	...
	if(vol->vol_src[VOL_C] == VOL_FROM_MEM){
		if (_abs( vol->vol_raw[VOL_C] - vol->vol_pre[VOL_C] ) >= VOL_TOLERANCE){
	...
	if(vol->vol_src[VOL_D] == VOL_FROM_MEM){
		if (_abs( vol->vol_raw[VOL_D] - vol->vol_pre[VOL_D] ) >= VOL_TOLERANCE){
