...
list_t list_create_static() {
	return ((list_t ) { .size = 0, .head = NULL, .tail = NULL } );
}
 ...
 */
//void list_iterate_over_nodes(const list_t* list,
//		list_predicate_t iterator, void* data) {
//	node_t* node = list->head;
//
 ...
 * @return
 */
void node_new_static(node_t* node, void* element) {
	node->element = element;
	node->next = NULL;
	node->prev = NULL;
}
...
//	return ((list_iterator_t ) { .current = (
//			node != NULL ? node :
